...
const UpdateNews: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (slug) {
        try {
          const news = new News();
          const data = await news.getOne(slug); // Assume getOne is a method to get promotion by slug
          console.log(data)
          if (data) {
            setFormData({
              title: data.title || '',
  ...
  return (
    <main className={Style.body}>
      <h2 className={FormStyles.h2}>แก้ไขข่าวสาร</h2>
      <ImageDrawer />
